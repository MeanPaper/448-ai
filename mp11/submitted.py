...
class q_learner():
    def __init__(self, alpha, epsilon, gamma, nfirst, state_cardinality):
        ...
        state_cardinality.append(3)
        self.Q = np.zeros(tuple(state_cardinality))
        self.Q_local = np.zeros(tuple(state_cardinality))
        self.N = np.zeros(tuple(state_cardinality))
    ...
    def choose_unexplored_action(self, state):
        ...
        '''
        # raise RuntimeError('You need to write this!')
        # the random generator will generate number from 0 to 2
        # the index to action convertion is the following
        # index_to_action = {0: -1, 1: 0, 2: 1} basically subtract 1 from the index


        # determine actions that do not meet the rquirements
        explore_count = self.report_exploration_counts(state)
        not_meet = []
        for i in range(len(explore_count)):
          if explore_count[i] < self.nfirst:
            not_meet.append(i)
        
        # check if there are any not met actions
        if len(not_meet) == 0:
          return None

        # find a random action, not_meet exists some actions
        action_index = random.randint(0, len(not_meet)-1) # max is 2, but the lenght has max of 3
        action = not_meet[action_index]
    ...
    def learn(self, state, action, reward, newstate):
        ...
        # action -1, 0, 1 map to 0, 1, 2
        # computation step, the formula
        q_state_action = self.report_q(state)[action + 1]
        q_state_action += (self.alpha * (self.q_local(reward, newstate) - q_state_action))

        # update
        self.Q[state[0], state[1], state[2], state[3], state[4], action + 1] = q_state_action
    ...
    def save(self, filename):
        ...
        '''
        # raise RuntimeError('You need to write this!')
        np.savez(filename, save_Q = self.Q, save_N = self.N)
    ...
    def load(self, filename):
        ...
        self.Q = load_data['save_Q']
        self.N = load_data['save_N']
    ...
    def exploit(self, state):
        ...
        '''
        # raise RuntimeError('You need to write this!')
        Qs = self.report_q(state)
        return (np.argmax(Qs)-1), np.max(Qs)
    ...
    def act(self, state):
        ...
        # 1. check nfirst times
        action = self.choose_unexplored_action(state) 
        if action is not None: return action
        
        # 2. check espilon, see if the a random number is above or below espilon
        rand_num = random.random()
        if rand_num < self.epsilon: # below the espilon threshold, random action
          new_act = random.randint(0,2)
          return new_act - 1
        else: # above espilon threshold, exploit
          new_act, _ = self.exploit(state)
          return new_act
